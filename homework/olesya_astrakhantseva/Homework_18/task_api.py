...
def create_post():
    ...
    assert response.status_code == 200, f'Ошибка при создании сущности, статус код - {response.status_code}'
    assert 'id' in response.json(), f'Тело ответа не содержит поле id'
...
def get_obj_id(post_id):
    ...
    assert response.status_code == 200, f'Ошибка при загрузке сущности, статус код - {response.status_code}'
    obj = response.json()
    return obj
...
def change_put(post_id):
    ...
    assert update_name == 'qa_put', f'Ожидалось "qa_put", но было получено {update_name}'
    assert update_data == {"capibara": "two"}, f'Ожидалось "{{capibara": "two"}}, но было получено {update_data}'
...
def change_patch(post_id):
    ...
    assert response.status_code == 200
    update_name = response.json().get('name')
    assert update_name == 'qa_patch', f'Ожидалось "qa_patch", но было получено {update_name}'


def delete_post(post_id):
    response = requests.delete(f'{base_url}/{post_id}', headers=headers)
    assert response.status_code == 200, f'Ошибка при удалении сущности, статус код - {response.status_code}'


def deleted_object(post_id):
    response = requests.get(f'{base_url}/{post_id}', headers=headers)
    assert response.status_code == 404, f'Сущность с id {post_id} должна быть удалена, но получен статус код {response.status_code}'
...
def main():
    ...
    print(f'Проверка удаленной сущности: успешно')
